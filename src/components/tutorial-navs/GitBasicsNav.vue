<template>
  <div class="menu tutorial-nav">
    <h2 class="nav-title"><i class="fab fa-git-alt git-icon"></i> Git Basics</h2>
    <ul class="menu-list">
      <li v-for="(item, index) in navItems" :key="index">
        <router-link v-if="!item.comingSoon" :to="item.path" :class="{ 'is-active': isActive(item.path) }">{{ item.title }}</router-link>
...
const navItems = getSectionNavItems('git-basics');
...
/* Git red */
.git-icon { color: #f34f29; margin-right: 0.4rem; }
.tutorial-nav { --tutorial-accent: #f34f29; }
</style>
