<template>
  <div class="project-notes-page">
    ...
    <h1 class="title is-1">Rotto Rocks CSS Project Notes</h1>
    <p class="subtitle is-4">
      Reference notes for the CSS-styled version of the Rotto Rocks tourism project.
    </p>
...
useHead({
  title: 'Rotto Rocks CSS Notes - GraphitEdge',
  meta: [
    {
      name: 'description',
      content: 'Rendered CSS project notes for the Rotto Rocks tourism project.',
    },
  ],
});
...
onMounted(async () => {
  try {
    const response = await fetch('/projects/css-basics/rotto-rocks/README.md');
    if (!response.ok) {
    ...
    loadError.value =
      error instanceof Error ? error.message : 'Could not load the CSS project notes.';
  } finally {
