<template>
  <div class="project-notes-page">
    ...
    <h1 class="title is-1">Black Swan Bistro HTML Project Notes</h1>
    <p class="subtitle is-4">
      Reference notes for the HTML-only version of the Black Swan Bistro project.
    </p>
...
useHead({
  title: 'Black Swan Bistro HTML Notes - GraphiteEdge',
  meta: [
    {
      name: 'description',
      content: 'Rendered HTML project notes for the Black Swan Bistro guided project.',
    },
  ],
});
...
onMounted(async () => {
  try {
    const response = await fetch('/projects/html-basics/black-swan-bistro/README.md');
    if (!response.ok) {
      throw new Error('Could not load the HTML project notes.');
    }
    markdownContent.value = await response.text();
  } catch (error) {
    loadError.value = error instanceof Error ? error.message : 'Could not load the HTML project notes.';
  } finally {
